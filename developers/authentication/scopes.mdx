...
- The member's **role sets the default ceiling** when they accept their invite. An owner can then tighten or widen it per member from the [Team](/using-zoop/team) page, in the **MCP & API access** dialog. **Demoting** a member (for example owner → tech) automatically narrows their ceiling to the new role's defaults, intersected with their current scopes — so a demotion can never leave them with more API or MCP access than the new role allows, while any owner-set scopes still within the new role's bounds are kept.
