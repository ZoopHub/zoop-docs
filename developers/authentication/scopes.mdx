...
<Callout type="warning">TODO(verify): As of the last source snapshot, all notes API routes are marked `internal` (session-only) and are not yet accessible via external tokens. Confirm whether `read:notes` / `write:notes` are available to API key or OAuth callers before publishing this claim.</Callout>
