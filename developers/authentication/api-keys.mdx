...
curl -X POST https://app.zoop.example/api/mcp \
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"customers_search","arguments":{"q":"Henderson"}}}'
```
...
`https://app.zoop.example` is a placeholder. Resolve your real host from `GET /.well-known/oauth-protected-resource` — never hardcode it.
...
### User keys are capped by the member's ceiling

A user key (`zoop_uk_…`) is a user-actor credential, so its scopes are bounded by the owning member's **ceiling** — the maximum external scopes that member is allowed (set by their role, adjustable by an owner). Two things follow:

- **Creation fails** if you request a scope beyond the member's current ceiling. Lower the request, or have an owner widen the member's ceiling first.
- **Effective scopes are re-checked on every request** as `granted ∩ ceiling`. If an owner lowers the member's ceiling later, the key is narrowed immediately — you do not need to recreate it.

Tenant keys (`zoop_tk_…`) are **not** bound to any member ceiling — they represent the account, not a person, so their scopes are exactly what you set at creation. See [scopes & permissions](/developers/authentication/scopes#the-per-member-ceiling) for the full model.
