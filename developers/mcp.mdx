...
The `get` and `create` tools for **customers, jobs, invoices, quotes, and plans** include a top-level `url` field in their result — a direct link to that record in Zoop, so a person can open it and confirm the work. The link has the form `https://app.zoop.pro/<tenant-id>/<resource>/<id>`, where `<resource>` is plural (for example `jobs` or `invoices`). Resolve the real host from `/.well-known/oauth-protected-resource` — `https://app.zoop.pro` is a placeholder.
