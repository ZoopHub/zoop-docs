...
All tools in this group require the `read:customers` or `write:customers` scope (a scope is a named permission you grant your API token — see [Scopes](/developers/authentication/scopes)). Contacts, locations, and tags share those two scopes; they do not have their own.
...
<Callout type="info">
Tool names appear here in their canonical dotted form (for example `customers.create`). When you call a tool, replace the dots with underscores — `customers_create`. See [tool names](/developers/concepts#tool-names). The example payloads below already use the wire form.
</Callout>

These are your account's own records: `customers.search` and `customers.get` return each customer's contact details — phone, email, and addresses — for you to use. You can also create or attach a customer by name (no UUID needed) from the [jobs](/developers/api/jobs#jobs-create), [invoices](/developers/api/invoices), [quotes](/developers/api/quotes), and [plans](/developers/api/plans) create tools — each accepts a `customer` object and finds or creates the matching customer for you.
...
Fetch one customer's full record by `id`, including their contacts and locations. The response includes a top-level `url` — a Verify on Zoop link straight to the customer, of the form `https://app.zoop.example/<tenant-id>/customers/<id>`.
...
You cannot change `customer_type` after creation, so pick the right one up front. The result includes a top-level `url` — a Verify on Zoop link straight to the new customer.
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "{\"id\":\"c9d8e7f6-...\",\"customer_type\":\"individual\",\"customer_number\":\"C-0042\",\"display_name\":\"Marco Reyes\",\"status\":\"active\",\"primary_phone\":\"+15105550199\",\"primary_email\":\"marco@example.com\",\"created_at\":\"2026-06-13T18:00:00.000Z\",\"url\":\"https://app.zoop.example/<tenant-id>/customers/c9d8e7f6-...\"}"
      }
    ]
  }
}
...
<Callout type="info">
`tags.create` requires a token that identifies a specific user. API keys that are scoped only to the account (with no associated user) cannot call this tool. Use a user-bound API key or an OAuth token instead. See [API keys](/developers/authentication/api-keys) if you are unsure which type you have.
</Callout>
