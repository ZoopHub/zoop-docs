...
<Callout type="info">
Tool names appear here in their canonical dotted form (for example `plans.create`). When you call a tool, replace the dots with underscores — `plans_create`. See [tool names](/developers/concepts#tool-names). The example payloads below already use the wire form.
</Callout>
...
The response includes a top-level `url` — a Verify on Zoop link to the plan. Returns a `not_found` error if the plan does not exist in this tenant.
...
<ParamField body="customer_id" type="string (UUID)">
  The customer this plan belongs to. Must exist in this tenant. Cannot be changed after creation. Provide either `customer_id` or `customer`.
</ParamField>

<ParamField body="customer" type="object">
  Identify the customer by name instead of by `customer_id`. Pass `{ "name": "...", "phone": "...", "email": "..." }` — only `name` is required. Zoop finds the matching customer or creates a new one. The `customer` path additionally requires both `read:customers` and `write:customers`. If the name matches more than one customer, the tool returns a `needs_customer` result (a `query` plus a `candidates` list of `id`/`display_name`/`primary_phone`/`primary_email`) instead of creating the plan — ask which one, then call again with `customer_id`. Pass `customer_id` or `customer`, not both.
</ParamField>
...
The result includes a top-level `url` (a Verify on Zoop link to the new plan, of the form `https://app.zoop.example/<tenant-id>/plans/<id>`) and `customer_created` — `true` when the `customer` path created a new customer.
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "{\"id\":\"p9a8b7c6-...\",\"title\":\"Monthly lawn care\",\"status\":\"active\",\"customer_id\":\"cccccccc-...\",\"cadence_rrule\":\"FREQ=MONTHLY;INTERVAL=1\",\"next_invoice_at\":\"2026-07-13T00:00:00.000Z\",\"lead_offset_days\":3,\"auto_charge_enabled\":false,\"items\":[{\"description\":\"Mow front and back\",\"quantity\":1,\"unit_price\":75,\"is_taxable\":true}],\"created_at\":\"2026-06-13T18:00:00.000Z\",\"customer_created\":false,\"url\":\"https://app.zoop.example/<tenant-id>/plans/p9a8b7c6-...\"}"
      }
    ]
  }
}
