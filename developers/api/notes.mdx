...
<Callout type="info">
Tool names appear here in their canonical dotted form (for example `notes.create`). When you call a tool, replace the dots with underscores — `notes_create`. See [tool names](/developers/concepts#tool-names). The example payloads below already use the wire form.
</Callout>
