...
<Callout type="info">
Tool names appear here in their canonical dotted form (for example `invoices.create`). When you call a tool, replace the dots with underscores — `invoices_create`. See [tool names](/developers/concepts#tool-names). The example payloads below already use the wire form.
</Callout>
...
Returns the full invoice record, including a top-level `url` — a Verify on Zoop link to the invoice. Returns `not_found` if the invoice does not exist in this tenant.
...
<ParamField name="customer_id" type="string (UUID)">
  The customer this invoice belongs to. Must belong to your tenant. A customer is required — pass either `customer_id` or `customer`.
</ParamField>

<ParamField name="customer" type="object">
  Identify the customer by name instead of by `customer_id`. Pass `{ "name": "...", "phone": "...", "email": "..." }` — only `name` is required. Zoop finds the matching customer or creates a new one. The `customer` path additionally requires both `read:customers` and `write:customers`. If the name matches more than one customer, the tool returns a `needs_customer` result (a `query` plus a `candidates` list of `id`/`display_name`/`primary_phone`/`primary_email`) instead of creating the invoice — ask which one, then call again with `customer_id`. Pass `customer_id` or `customer`, not both.
</ParamField>
...
Returns the created invoice record. The `status` is `"draft"` and the `total`, `subtotal`, and `tax_amount` fields contain the server-computed values. The result also includes a top-level `url` (a Verify on Zoop link to the new invoice, of the form `https://app.zoop.pro/<tenant-id>/invoices/<id>`) and `customer_created` — `true` when the `customer` path created a new customer.
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "{\"id\":\"iiiiiiii-iiii-iiii-iiii-iiiiiiiiiiii\",\"tenant_id\":\"tttttttt-tttt-tttt-tttt-tttttttttttt\",\"customer_id\":\"cccccccc-cccc-cccc-cccc-cccccccccccc\",\"invoice_number\":\"INV-0001\",\"status\":\"draft\",\"subtotal\":230.00,\"tax_amount\":19.00,\"total\":249.00,\"issued_at\":\"2026-06-13T00:00:00Z\",\"due_at\":\"2026-07-13T00:00:00Z\",\"notes\":\"Thank you for the work order.\",\"created_at\":\"2026-06-13T10:00:00Z\",\"customer_created\":false,\"url\":\"https://app.zoop.pro/<tenant-id>/invoices/iiiiiiii-iiii-iiii-iiii-iiiiiiiiiiii\"}"
      }
    ]
  }
}
