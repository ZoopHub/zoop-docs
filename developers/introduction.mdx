...
When you make the call, you send the tool's **wire name** — the dotted canonical name with each dot replaced by an underscore (`customers.search` → `customers_search`). See [tool names](/developers/concepts#tool-names).
...
Replace `zoop_uk_<secret>` with your actual API key, and replace `https://app.zoop.example` with your Zoop app host (resolve the real host from `GET /.well-known/oauth-protected-resource`).
