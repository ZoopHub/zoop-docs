...
**Base URL:** examples use the production base URL `https://app.zoop.pro`. For another environment (staging or local), fetch `GET /.well-known/oauth-protected-resource` — the `resource` field is the canonical base URL for whatever environment you're hitting.
