...
**Base URL:** examples use the placeholder base URL `https://app.zoop.example`. For your actual environment (production, staging, or local), fetch `GET /.well-known/oauth-protected-resource` — the `resource` field is the canonical base URL for whatever environment you're hitting.
...
<Callout type="info">
You call tools by their **wire name**: the dotted canonical name with each dot turned into an underscore, so `customers.search` becomes `customers_search`. See [tool names](/developers/concepts#tool-names).
</Callout>
